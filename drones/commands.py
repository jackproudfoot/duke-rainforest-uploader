...
def list():
    ...
    res = requests.get(urljoin(os.environ.get('API_URL'), '/drone/get_drones'))

    drones = json.loads(res.text)['results']
...
def create(name, id, make, model):
    ...
    res = requests.post(urljoin(os.environ.get('API_URL'), '/drone/create_drone/'), json={
        'name': name,
        'drone_id': id,
        'make': make,
        'model': model
    ...
    click.echo('Drone created with uid: {}'.format(res.json()['drone_uid']))